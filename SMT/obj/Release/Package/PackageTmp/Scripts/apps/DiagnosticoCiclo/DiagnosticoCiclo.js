var DiagnosticoCiclo = new function () {
    var _a = this;
    ...
    $("#tabla-diagnostico-ciclo").keydown(function (e) {
                ...
                if (previo == undefined || previo.data("total-agregado") != undefined) {
                    ...
                    do {
                        ...
                        if (previo.data("total-agregado") != undefined) {
                            ...
                            contador = 1;
                        } else if (previo.length == 0 && $(e.target).closest('td').next().find("[data-diagnostico-ciclo-total]").length==0) {
                            $(anterior).children().eq(cellIndex + 1).find("input").focus()
                            contador = 1;
                        } else if ($(e.target).closest('td').next().find("[data-diagnostico-ciclo-total]").length > 0) {
                            $(e.target).closest('tr').next().next().children().eq(7).find("input").focus();
                            contador = 1;
                        }
                        c++;
                        if (c == 10) {
    ...
    var generarNombreCache = function () {
        return 'diagnosticociclo-' + _grupo;
    }
    ...
    this.generar = function (selector, diagnostico, focus, autoOrdenar) {

        var elementosDiagnosticoCiclo = $(selector).find('tbody');
       
        diagnostico.Temas.map(function (tema) {
            tema.num = 0;
            tema.IDDiagnosticoCiclo = diagnostico.IDDiagnosticoCiclo;
            tema.Tipo = diagnostico.Tipo;
            tema.Titulo = diagnostico.Titulo;
            tema.FechaEntregaDesplegable = diagnostico.FechaEntregaDesplegable;
            tema.FechaEntrega = diagnostico.FechaEntrega;
            tema.tds = tdsCaptura.format(tema);

            var element = $(rowDiagnosticoCiclo.format(tema)).appendTo(elementosDiagnosticoCiclo).addData(tema);

            $(elementosDiagnosticoCiclo).append(element);

            tema.Alumnos.map(function (al) {
                elementosDiagnosticoCiclo.find('[data-alumno-id="' + al.IDAlumno + '"][data-alumno-tema="' + tema.IDTema + '"] input').val(al.Calificacion);
            });

            $(elementosDiagnosticoCiclo).find('[title]').tooltip();

            if($('body').hasClass('visualizando'))
                $(elementosDiagnosticoCiclo).find('input').attr('disabled', true);
        });

        if (focus == true) {
            $('html, body').animate({
                scrollTop: $(selector).find('[data-diagnostico-ciclo-id="' + diagnostico.IDDiagnosticoCiclo + '"]:first').offset().top
            }, 1000);

            $(selector).find('[data-diagnostico-ciclo-id="' + diagnostico.IDDiagnosticoCiclo + '"]:first').resaltar('info', 3000)
                        .find('input:first')
                        .focus();
        }
    ...
    this.desplegarResultados = function (grupo, selector) {
        ...
        Alumnos.listar(_grupo).then(function () {
            ...
            Loading('Cargando diagnosticos');
            Templates.load('tablaDiagnosticoCiclo', '/Scripts/apps/DiagnosticoCiclo/views/tablaDiagnosticoCiclo.html').then(function (template) {
    ...
    var actualizarCalificacion = function (alumno, calificacion, tema,diagnostico) {
        ...
        var input = $('[data-diagnostico-ciclo-id] [data-alumno-id="' + alumno + '"][data-alumno-tema="' + tema + '"] input');

        $.ajax({
            url: '/DiagnosticoCiclos/actualizarCalificacion',
            type: 'post',
            data: {
                alumno: alumno,
                tema: tema,
            ...
            beforeSend: function () {
                $('[data-diagnostico-ciclo-id] [data-alumno-id="' + alumno + '"][data-alumno-tema="' + tema + '"]').html('<span class="fa fa-spin fa-refresh"></span>');
            },
            complete: function () {
                $('[data-diagnostico-ciclo-id] [data-alumno-id="' + alumno + '"][data-alumno-tema="' + tema + '"] span.fa-spin').remove();
            },
            success: function (response) {
                if (response.result == true) {
                    // Se despliega la sesion actualizada con efecto
                    $('[data-diagnostico-ciclo-id] [data-alumno-id="' + alumno + '"][data-alumno-tema="' + tema + '"]')
                        .html(input)
                        .resaltar('info', 800);
                    ...
                    actualizarTotales(diagnostico);
                    Alumnos.cargarSemaforo('diagnosticos', alumno);
                }
                else {
                    AlertError(response.message);
                    $('[data-diagnostico-ciclo-id] [data-alumno-id="' + alumno + '"]').focusin();
                }
            }
        })
    ...
    var actualizarTotales = function (id) {
        var selector = $('[data-tabla="diagnostico-ciclo"]');


        // Calcular total de rectivos de un diagnostico
        $(selector).find('[data-total-reactivos="'+id+'"]').each(function () {
            var total = 0;

            $('[data-diagnostico-ciclo-id="'+ id +'"]').find('[data-reactivos]').each(function () {
        ...
        $(selector).find('[data-diagnostico-ciclo-promedio="' + id + '"]').each(function () {
            ...
            $(this).parents('tr').find('[data-diagnostico-ciclo-total="' + id + '"]').html(sumatoria);
        ...
        // Calificaciones de todos los temas de cada alumno en el diagnostico
        $(selector).find('[data-diagnostico-ciclo-id="' + id + '"][data-total-agregado] [data-alumno-total]').each(function () {
            var numeroAspectos = $(selector).find('[data-diagnostico-ciclo-id="' + id + '"]').length - 1;
            var calificacion = 0;
            $(selector).find('[data-diagnostico-ciclo-id="' + id + '"] [data-alumno-id="' + this.getAttribute('data-alumno-total') + '"][data-alumno-tema] input').each(function () {
        ...
        $(selector).find('[data-alumno-aprobado]').each(function () {
            ...
            $(selector).find('[data-diagnostico-ciclo-id][data-total-agregado] [data-alumno-total="' + alumno + '"]').each(function () {
    ...
    var obtenerPosicion = function (selector, fecha) {
        ...
        var list = $(selector).find('[data-diagnostico-ciclo-fecha][data-visible="true"]').get();
        var date = fecha.toDate2();
        var tr = undefined;
        for (var m = 0; m < list.length - 1; m++) {

            if (date <= list[m].getAttribute('data-diagnostico-ciclo-fecha').toDate2() && date > list[m + 1].getAttribute('data-diagnostico-ciclo-fecha').toDate2()) {
                tr = list[m + 1];
                break;
            }
            else if (date > list[m].getAttribute('data-diagnostico-ciclo-fecha').toDate2() && m == 0) {
                tr = list[m];
                break;
            }
        }
    ...
    var ajustarRows = function (selector) {
        ...
        var ids = [];
        $(selector).find('[data-diagnostico-ciclo-id]').each(function () {
            if (ids.indexOf(this.getAttribute('data-diagnostico-ciclo-id')) == -1) ids.push(this.getAttribute('data-diagnostico-ciclo-id'));
        });
        ...
        ids.map(function (id) {
            ...
            // agregar row para totales a cada conjunto de diagnostico
            $(selector).find('[data-diagnostico-ciclo-id="' + id + '"]:last').after('<tr data-total-agregado="" data-diagnostico-ciclo-id="' + id + '"><td colspan="3" rowspan=""></td><th><div class="w100">Calificación</div></th><td><div data-total-reactivos="'+id+'" class="w100">0</div></th>' + tdsTotales + '</tr>');

            var total = $(selector).find('[data-diagnostico-ciclo-id="' + id + '"]').length;

            $(selector).find('[data-diagnostico-ciclo-id="' + id + '"]').each(function (i, k) {
                if (i == 0) {
                    $(k).attr('data-visible', true);
                    $(k).find('td[rowspan]').removeClass('hide').attr('rowspan', total);
                    $(k).find('td:first').html('<div class="w50"><a><span data-option="download" class="fa fa-download visor-oculto" title="Descargar diagnostico"></span></a><a><span data-option="editar" class="fa fa-edit visor-oculto" title="Editar diagnostico"></span></a><a><span data-option="eliminar" class="fa fa-trash visor-oculto" title="Eliminar diagnostico"></span></a> ' + index + '</div>');
                    index++;
                }
                else {
    ...
    this.nuevo = function (selector, estado) {
        ...
        Templates.load('nuevoDiagnosticoCiclo', '/DiagnosticoCiclos/nuevo').then(function (template) {
            Loading();
            ConfirmDialog.show({
                ...
                beforeOpen: function () {
                    ...
                    form.find('[data-option="pregunta-ciclo"]').click();

                    form.find('[name="Titulo"]').val('Diagnósticos ' + _bimestre);
                    ...
                    form.submit(function (e) {
                        ...
                        if ($(this).find('[data-pregunta]').length == 0) {
                            AlertWarning('Debes capturar por lo menos una pregunta', 'Diagnosticos por Ciclo');
                            return;
                        }
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.result == true) {
                                        ...
                                        AlertError(response.message, 'Diagnosticos por Ciclo');
                                    }
                    ...
                    form.submit(function (e) {
                        ...
                        if ($(this).valid()) {
                            $.ajax({
                                ...
                                success: function (response) {
                                    if (response.result == true) {
                                        $('[data-diagnostico-ciclo-id="' + response.data.IDDiagnosticoCiclo + '"]').remove();
                                        _a.generar(selector, response.data, true, true);
                                        ConfirmDialog.hide();
    ...
    this.Imprimir = function () {
        open('/diagnosticoCiclos/imprimir?grupo=' + _grupo + '&bimestre=' + _bimestre);
    }

    this.eliminar = function (selector, id) {
        ConfirmDialog.show({
            title: 'Eliminar diagnostico ciclo',
            text: '<h3 class="text-center">Esta intentando eliminar este diagnostico permanentemente, el cual ya no se podrá recuperar. ¿Desea continuar?</h3>',
            ...
            callback: function (result) {
                if (result == true) {
                    $.ajax({
                        url: '/DiagnosticoCiclos/eliminar',
                        type: 'post',
                        data: {
                            id: id
                        },
                        ...
                        success: function (response) {
                            if (response.result == true) {
                                $('[data-diagnostico-ciclo-id="' + id + '"]').removeConEfecto();
                                AlertSuccess('Se ha eliminado el diagnostico', 'Diagnosticos cicloes');
                                ConfirmDialog.hide();
                                actualizaDataEnCache();
                                DiagnosticoCiclo.desplegarResultados(_grupo, selector);
                            }
                            else {
                                AlertError(response.message, 'Diagnosticos cicloes');
                            }
    ...
    var ajustarIndex = function () {
        $('#frmDiagnosticoCiclo').find('[data-pregunta]').each(function (i,k) {
            $(this).find('[name]').each(function () {
                this.name = 'DiagnosticoCicloTema['+ i +']' + this.name.substring(this.name.indexOf('.'),this.name.length);
            });
        });
    }
    ...
    $('body:not(.visualizando)').delegate('[data-alumno-id][data-alumno-tema] input', 'change', function () {
        ...
        if ($(this).val() >= 0 && $(this).val() <= 100) {
            ...
            actualizarCalificacion(alumno, $(this).val(), tema, $(this).parents('[data-diagnostico-ciclo-id]').attr('data-diagnostico-ciclo-id'));
    ...
    $('body:not(.visualizando)').delegate('[data-diagnostico-ciclo-id] [data-option="eliminar"]', 'click', function () {
        var tr = $(this).parents('[data-diagnostico-ciclo-id]');
        _a.eliminar(tr.parents('table'), tr.attr('data-diagnostico-ciclo-id'));
    });
    $('body:not(.visualizando)').delegate('[data-diagnostico-ciclo-id] [data-option="editar"]', 'click', function () {
        var tr = $(this).parents('[data-diagnostico-ciclo-id]');
        var data = [];

        $('[data-diagnostico-ciclo-id="' + tr.attr('data-diagnostico-ciclo-id') + '"]:not([data-total-agregado])').each(function () {
    ...
    // Agregar preguntas
    $('body:not(.visualizando)').delegate('[data-option="pregunta-ciclo"]', 'click', function () {
        ...
        $.validator.unobtrusive.parse($('#frmDiagnosticoCiclo'));
    });

    $('body:not(.visualizando)').delegate('#frmDiagnosticoCiclo [data-option="eliminar-pregunta"]', 'click', function () {
    ...
    $('body:not(.visualizando)').delegate('[data-option="download"]', 'click', function () {
        ...
        open('/DiagnosticoCiclos/download?diagnostico=' + $(this).parents('[data-diagnostico-ciclo-id]').attr('data-diagnostico-ciclo-id'));
    });

    // Se precarga template 
    Templates.load('rowDiagnosticoCiclo', '/Scripts/apps/DiagnosticoCiclo/views/rowDiagnosticoCiclo.html').then(function (template) {
        rowDiagnosticoCiclo = template;
...
$('body:not(.visualizando)').delegate('[data-diagnostico-ciclo="nuevo"]', 'click', function () {
    DiagnosticoCiclo.nuevo('#tabla-diagnostico-ciclo', 1);
});
